...
<div id="layout_conteudo">
    <div class="justify-content-center">
        <div class="col-md-12">
            ...
            @if ($room->schoolclasses->isNotEmpty())
                    ...
                    <table class="table table-bordered" style="font-size:15px;">
                        ...
                        @foreach($horarios ? range(0, count($horarios)-2) : [] as $x)
                            <tr>
                                    ...
                                    @foreach($dias as $dia)
                                        @php
                                            $slot = $slots[$dia][$horarios[$x]] ?? null;
                                            $turma = $slot['class'] ?? null;
                                        @endphp

                                        @if($turma)
                                            @php
                                                $classschedule = $slot['schedule'] ?? null;
                                                $excesao = $turma->classschedules->first(function($cs) use($dia, $horarios, $x) {
                                                    return $cs->diasmnocp == $dia && $cs->horsai == $horarios[$x+1];
                                                });
                                                $excesao2 = $turma->classschedules->first(function($cs) use($dia, $horarios, $x) {
                                                    return $cs->diasmnocp == $dia && $cs->horent < $horarios[$x] && $cs->horsai > $horarios[$x+1];
                                                });
                                            @endphp
                                            @if($classschedule)
                                                @php $i+=1; @endphp
